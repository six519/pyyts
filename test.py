#!/usr/bin/env python

import pyyts

test_client = pyyts.YTSClient("test")
#print test_client.movies.list_movies()
#print test_client.movies.movie_details()
#print test_client.movies.movie_suggestions(movie_id=10)
#print test_client.movies.movie_comments()
#print test_client.movies.movie_reviews(movie_id=20)
#print test_client.movies.movie_parental_guides(movie_id=20)
#print test_client.movies.list_upcoming()
#print test_client.users.user_details(user_id=16)
#print test_client.users.user_forgot_password()
print test_client.users.user_get_key()